 ...
 * @param compression - Compression type to use (none, gzip, brotli, zstd)
 ...
 * @param compression - Compression type that was used (none, gzip, brotli, zstd)
...
export async function restoreFromFileStreamingOptimized(
  ...
  try {
    ...
    if (compression === "none") {
      ...
      const decoded = await optimizedMsgPackDecode(finalBuffer);
      ...
      log.info(`⏱️ Optimized restore timing breakdown: Zstd decompress: ${msgpackDecodeStart - streamStart}ms, MsgPack decode: ${msgpackDecodeTime}ms, Orama load: ${oramaLoadTime}ms`);
      return db as OramaSearchDatabase;
    } else {
      // Create decompression stream for gzip/brotli
